...
горим = read.csv("D:/VIP/bin/RServer/KAZ_data-1.csv", sep=";")
горим

qwAll <- c("r1","r2","r3","r4","r5","r6")
qwUP <-  c("r1","r2")
qwDown <- c("r3","r4","r5","r6")
...
colnames(tmp1)=qwwLeft; row.names(tmp1)=qwAll
...
for (i in seq(1, nrow(tempdata2), by = 1)) { for (j in seq(1, ncol(tempdata2), by = 1)){ tmp1[j,i]= tempdata2[[i,j]]    }  }
F <- c(1:4)
for (i in seq(1, 4, by = 1)) {  F[i]=(28.55 - 0.61 * mt[4,i]);  mtR[1,i]=F[i]}

S<- c(1:4); for (i in seq(1, 4, by = 1)) {  S[i]=(F[i]*0.0123+mt[2,i]*0.033+mt[3,i]*0.466+mt[4,i]*0.0684+1.324) ;  mtR[2,i]=S[i]}
